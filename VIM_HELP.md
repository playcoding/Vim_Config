...
### 搜索
    ...
    /pattern   搜索 pattern 的字符串（如果搜索出多个，可按n或N键到下一个或上一个）  
...
例：假设你有一个字符串 (map (+) ("foo")).而光标键在第一个 o 的位置。
...
> 在Windows下的vim，你需要使用 `<Ctrl-q>` 而不是 `<Ctrl-v>` ，`<Ctrl-v>` 是拷贝剪贴板。

可视化选择： `v`,`V`,`<Ctrl-v>`
前面，我们看到了 `<Ctrl-v>` 的示例（在Windows下应该是`<Ctrl-q>`），我们可以使用 `v` 和 `V`。也可以用 `Vip` 选择一个段落里的所有行，一但被选好了，你可以做下面的事：
...
多行的行首添加内容
    ...
    ^           到行头
    <Ctrl-v>    开始块操作
    <Ctrl-d>    向下移动 (你也可以使用 `hjkl` 来移动光标，或是使用 `%`，或是别的)
    `I--` `[ESC]`   `I` 是在行首插入，`--`是要插入的内容，按 `ESC` 键两次来为每一行生效。
...
多行的行尾添加内容
    ...
    $           到行最后
    `A--` `[ESC]`   `A` 是在行尾添加，`--`是要添加的内容，按 `ESC` 键两次来为每一行生效。
...
### 自动补全
    ...
    Ctrl-p(Ctrl-n):快速补全命令
> 在编辑模式中,输入几个字符后再输入此命令则开始向上(下)搜 索开头与其匹配的单词并补齐, 不断输入此命令则循环查找. 此命令会在 vim 程序当前打开的所有文件中进行匹配.

    Ctrl-x_Ctrl-l（输入Ctrl-x后接着输入Ctrl-l）:自动补全一行的内容
    只在当前文档查找匹配的行,多个选项时通过快速补全命令选择.
    
    Ctrl-x_Ctrl-f（输入Ctrl-x后接着输入Ctrl-f）:自动补全目录
    如输入 c:\p,然后按Ctrl-x Ctrl-f, 则得到 C:\Program Files\ 

### 代码输入模版

    abbr : 缩写设置
    例:abbr cmain int main(int argc, char** argv),则下次只要输入cmain后再输入其他非字母符号, 它就会自动扩展为int int main(int argc, char** argv)
